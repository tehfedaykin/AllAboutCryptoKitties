...
test.run('http://localhost:4200/about', function(error, results) {
	if (error) {
		return console.error(error.message);
	}

  var htmlResults = htmlReporter.process(results, 'http://localhost:4200/about');

  fs.writeFile('examples/output/report-about.html', htmlResults, function(err) {})
